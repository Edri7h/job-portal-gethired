...
export default function SignupForm() {
  ...
  const changeFileHandler = (e) => {
    setfieldData({ ...fieldData, file: e.target.files?.[0] })
  };
  ...
  const submitHandler = async (e) => {
    ...
    formData.append("fullname", fieldData.fullname)
    formData.append("email", fieldData.email)
    formData.append("phoneNumber", fieldData.phoneNumber)
    formData.append("password", fieldData.password)
    formData.append("role", fieldData.role)
    if (fieldData.file) {
    ...
    <>
      ...
      <div className="flex justify-center items-center min-h-screen bg-gray-50 p-4">
        <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
          <h1 className="text-xl font-bold text-gray-800 mb-4">Sign Up</h1>
          ...
          <form onSubmit={submitHandler} className="space-y-4">
            ...
            <div className="flex space-x-4 items-center">
              ...
              <div className="flex-1 flex items-center px-3 py-2 border border-gray-300 border-dashed rounded-md">
                <label htmlFor="file-upload" className="relative cursor-pointer bg-white rounded-md font-medium text-blue-600 hover:text-blue-500 text-sm mr-2">
                  Choose File
                  <input onChange={changeFileHandler}
                    id="file-upload" name="file-upload" accept='image/*' type="file" className="sr-only" />
                </label>
                <span className="text-xs text-gray-500">
                  {fieldData.file ? fieldData.file.name : "No file chosen"}
                </span>
            ...
            {
            loading?(<button className="flex justify-center items-center w-full py-2 px-4 bg-blue-800 hover:bg-blue-900 text-white font-medium rounded-md shadow-sm transition-colors text-sm"><Loader2 className='animate-spin mr-2 h-4 w-4'/>please wait</button>):(
              <button
              type="submit"
